...
class ProductDetailsFragment : Fragment(){
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        arguments?.run {
            ...
            viewModel.loadProductDetails(id).observe(viewLifecycleOwner) { product ->
                if (product != null) {
    ...
    private fun setShareIntent(){
        arguments?.run {
            val id = getLong(EXTRA_PRODUCT_ID, -1)
            viewModel.loadProductDetails(id).observe(viewLifecycleOwner) { productLoaded ->
                val text =
                    getString(R.string.share_text, productLoaded?.name, productLoaded?.description)
